...
Add column (time) 5 min interval in a day. This is for the convenience in calculating average daily activity pattern. 
```{r,results='hide'}
Dt$date=ymd(Dt$date)          # Convert to date type
Dt[,datetime:=date+hours(interval %/% 100)+minutes(interval %% 100)] # Add time part
Dt[,time:= ymd(Sys.Date())+hours(interval %/% 100)+minutes(interval %% 100)] 
