...
```{r}
setwd('C:\\Users\\Ziwen.Yu\\Documents\\Class\\Reproducible research\\Assignment 2\\')
...
```{r,message=FALSE,results='hold'}
# Read data from csv file and choose the necessary fields for further analysis
setwd('C:\\Users\\Ziwen.Yu\\Documents\\Class\\Reproducible research\\Assignment 2\\')
if (!"Storm" %in% ls()) Storm=read.csv('repdata-data-StormData.csv',sep=',',stringsAsFactors=F)
dim(Storm)
```
...
ggplot(data=Storm1)+
      geom_histogram(aes(year(BGN_DATE)))+
      xlab('')+
      ggtitle('Histogram of Sever Weather Event')+
      Plot_theme
```
...
Storm1$PropDMG_Dollar=apply(Storm1[,c('PROPDMG','PROPDMGEXP')]
                            ,1
                            ,function(x) (as.numeric(x[1])*10^Dmgscl(x[2]))
                            )

Storm1$CropDMG_Dollar=apply(Storm1[,c('CROPDMG','CROPDMGEXP')]
                            ,1
                            ,function(x) (as.numeric(x[1])*10^Dmgscl(x[2]))
                            )
```
...
DMG=data.table(Storm1[,c('EVTYPE','FATALITIES','INJURIES','PropDMG_Dollar','CropDMG_Dollar')])
...
# scale on y axis
Scl=c(0,ceiling(max(Results$PropDMG_Dollar,Results$CropDMG_Dollar)/1e+9/50)*50)
# Top 10 EVTYPE on property damage
TopTYPE=Results$EVTYPE[order(Results$PropDMG_Dollar,decreasing=T)[1:10]]
PropDMGPlot=ggplot(data=PlotDt[PlotDt$variable=='PropDMG_Dollar',]
       ,aes(x=reorder(EVTYPE, -value), y=value/1e+9))+
...
CropDMGPlot=ggplot(data=PlotDt[PlotDt$variable=='CropDMG_Dollar',]
       ,aes(x=reorder(EVTYPE, -value), y=value/1e+9))+
      geom_bar(stat="identity")+
      xlab('')+
      ylab('Damage ($ Billion)')+
...
FATAPlot=ggplot(data=PlotDt[PlotDt$variable == 'INJURIES',]
       ,aes(x=reorder(EVTYPE, -value), y=value/1000))+
      geom_bar(stat="identity")+
      ylab('Lives (k)')+
      xlab('')+
      ggtitle('Total Fatality by\n Severe Weather Event')+
      xlim(TopTYPE)+
      ylim(Scl)+
      Plot_theme
...
INJPlot=ggplot(data=PlotDt[PlotDt$variable == 'FATALITIES',]
                ,aes(x=reorder(EVTYPE, -value), y=value/1000))+
      geom_bar(stat="identity")+
      ylab('Lives (k)')+
      xlab('')+
      ggtitle('Total Injury by\n Severe Weather Event')+
      xlim(TopTYPE)+
      ylim(Scl)+
      Plot_theme
